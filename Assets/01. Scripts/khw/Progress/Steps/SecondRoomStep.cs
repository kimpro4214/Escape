...
public class SecondRoomStep : ProgressStepBase
{
    ...
    {
        VoiceManager.Instance.AddVoice(new VoiceLine(null, ESubtitleCharacters.witch, "하나 더 찾아냈네, 완성한다면 어떤 모양일지 생각해봐.", 0f, null));
    }

    private void OnThirdInfoFound()
    {
        VoiceManager.Instance.AddVoice(new VoiceLine(null, ESubtitleCharacters.witch, "다 찾은 것 같네, 마법진을 그릴 시간이야.", 0f, null));
    }
